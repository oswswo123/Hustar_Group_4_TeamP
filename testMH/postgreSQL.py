...
class PostgresDB:
    def __init__(self, host='localhost', dbname='ainalyst', user='hustar', password='1234', port=5432):  # defalut = ainalyst
        try:
            ...
            print(f"ERROR:: Connection Error for Postgres Database... !! (Error code: {e})")
        self.host = host
        self.dbname = dbname
        self.user = user
        self.port = port
...
def main():
    ...
    ainalyst.execute(f"INSERT INTO report VALUES('{data['id']}', '{data['create_date']}', '{data['subject']}',"
                     f"'{data['price']}', '{data['opinion']}', '{data['writer']}', '{data['stock_firm']}')")
