...
{ // x값이 큰 좌표가 왼쪽 눈이므로 x값이 큰 좌표에서 x 값이 작은 좌표 뺀다.
	Point2d delta = (pt[0].x > pt[1].x) ? pt[0] - pt[1] : pt[1] - pt[0];
	double angle = fastAtan2(delta.y, delta.x); // 차분으로 기울기 계산

	Mat rot_mat = getRotationMatrix2D(face_center, angle, 1);  // 회전 변환 행렬 반환
	return rot_mat;
}
...
Mat correct_image(Mat image, Mat rot_mat, vector<Point2d>& eyes_center)
{
	...
	{
		Point3d coord(eyes_center[i].x, eyes_center[i].y, 1); // 행렬 곱위해 3차원 좌표로
		Mat dst = rot_mat * (Mat)coord;
		eyes_center[i] = (Point2d)dst; // 눈 좌표 저장
	}
	return correct_img;
}