...
from transformers import AutoModel, AutoTokenizer, AutoModelForSequenceClassification, AutoConfig, DataCollatorWithPadding
...
def main():    
    ...
    for pretrained in pretrained_model:
        inference_dataloader = module.create_dataloader(data_file_path, pretrained, pretrained_model[pretrained])
    ...
    for probabilities in all_models_probabilities:
        voting_probabilities.append(module.soft_voting(probabilities))
        
    module.save_csv_file(data_file_path, save_file_path, voting_probabilities)
