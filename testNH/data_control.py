...
def data_split(all_data, data_path, fold_length):
    train_data, test_data = train_test_split(all_data, test_size=0.2, random_state=42, stratify=all_data[["labels"]])

    X = train_data.drop(labels=["labels"], axis=1)
    y = train_data["labels"]
...
def load_csv_to_dataset(model_path, pretrained_model, fold_length):
    if pretrained_model == "kb-albert-char-base-v2":
    ...
    for fold_number in range(1, fold_length+1):
        ...
        train_processed = train_dataset.map(preprocessing)
        valid_processed = valid_dataset.map(preprocessing)

        train_dataset = train_processed.map(tokenized_fn, remove_columns=["article"], fn_kwargs={"tokenizer": tokenizer, "max_len": MAX_LEN})
        valid_dataset = valid_processed.map(tokenized_fn, remove_columns=["article"], fn_kwargs={"tokenizer": tokenizer, "max_len": MAX_LEN})

        dataset_list.append([train_dataset, valid_dataset])

    test_dataset = load_dataset("csv", data_files=f"./data/test_data.csv")["train"]
    test_processed = test_dataset.map(preprocessing)
    test_dataset = test_processed.map(tokenized_fn, remove_columns=["article"], fn_kwargs={"tokenizer": tokenizer, "max_len": MAX_LEN})
...
def create_inference_dataloader(data_file_path, pretrained, pretrained_model_path):
    ...
    processed = inference_dataset.map(preprocessing)
    inference_dataset = processed.map(lambda data: tokenizer(data["article"], padding="max_length", max_length=MAX_LEN, truncation=True),
                                              remove_columns=["company", "title", "opinion", "firm", "date", "article"])
    data_collator = DataCollatorWithPadding(tokenizer=tokenizer)
