...
def preprocessing(data, companylist):
    articles = []
    for article in data['article']:
        article = re.sub("\([^)]*\d{6}[^)]*\)", " ", article)
        article = re.sub("\n", " ", article)
        for c in companylist: # 기업명 제거
    ...
    for article in articles:
        ...
        data['article'] = processed
        data.to_csv('inferencedatasetprocessed.csv', index=False)

data = pd.read_csv('report_dataset.csv')
companylist = makecompanylist(data)
preprocessing(data, companylist)