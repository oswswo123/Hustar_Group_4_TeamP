...
config = AutoConfig.from_pretrained('nile/roberta-large-finetuned-wholemasking20')
config.num_labels = 2
load_model = AutoModelForSequenceClassification.from_pretrained('nile/roberta-large-wholemasking20', config=config)
load_model.to(device)
load_model.load_state_dict(torch.load("/content/drive/MyDrive/reviewdataset/roBERTatrain.pt"))
...
inference_dataframe.to_csv(f"/content/drive/MyDrive/reviewdataset/roBERTainferenced.csv", index=False)