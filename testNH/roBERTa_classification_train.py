...
config.num_labels = 2
model = AutoModelForSequenceClassification.from_pretrained('nile/roberta-large-finetuned-wholemasking20', config=config)
MODEL_NAME = "roBERTa-large-DAPT"
    ...
    with torch.no_grad():
        ...
        if i > 5:
            if loss_log[i] < loss_log[i - 1] and loss_log[i] < loss_log[i - 2] and loss_log[i] < loss_log[i - 3]:
                torch.save(model.state_dict(), f"/content/drive/MyDrive/reviewdataset/roBERTatrain.pt")
