...
def setmodel():
    ...
    config.num_labels = 2
    model = AutoModelForSequenceClassification.from_pretrained('nile/roBERTa-finetuned-wholemasking20', config=config)
    return model
...
def train(model, train_dataloader, valid_dataloader):
    ...
    for i in range(num_epochs):
        ...
        for j, v in progress_bar:
            input_ids, attention_mask, labels = v['input_ids'].to(device), v['attention_mask'].to(device), v[
                'labels'].to(
                device)
        ...
        with torch.no_grad():
            ...
            if i > 5:
                if loss_log[i] < loss_log[i - 1] and loss_log[i] < loss_log[i - 2] and loss_log[i] < loss_log[i - 3]:
                    torch.save(model.state_dict(), "roBERTatrain.pt")
                if acc_log[i] <= acc_log[i - 1] and acc_log[i - 1] <= acc_log[i - 2] and acc_log[i - 2] <= acc_log[
                    i - 3]:
                    break

set_seeds()
file = "traindataset.csv"
train_dataloader, valid_dataloader = loadtraindataset(file)
model = setmodel()
